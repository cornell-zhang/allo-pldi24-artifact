...
def draw_plots(
    data, ylabel_fontsize=20, xticks_fontsize=20, yticks_fontsize=16, legend_fontsize=18
):
    keys = list(data.keys())
    ...
    for i, cat in enumerate(categories):
        bars = ax1.bar(
            [pos + width / 2 + width * i for pos in x],
            values[cat],
            width=width,
            label=cat,
    ...
    ax1.set_xticklabels(
        keys, rotation=0, ha="center", fontsize=xticks_fontsize
    ...
    legend = ax1.legend(
        loc="upper right", bbox_to_anchor=(0.9, 1), fontsize=legend_fontsize, ncol=6
    )
    ...
    # Save to file
    plt.savefig("polybench.pdf", format="pdf", dpi=200, bbox_inches="tight")
    # Print a message
    print("Plot saved to file: polybench.pdf")
