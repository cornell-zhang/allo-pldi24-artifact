...
void S0(
  ...
  l_update_i1: for (int i1 = 0; i1 < 200; i1++) {	// L4
    float v3[240];	// L5
    #pragma HLS array_partition variable=v3 complete dim=1 // manually added
    l_j1_init: for (int j1_init = 0; j1_init < 240; j1_init++) {	// L7
    #pragma HLS pipeline II=1
    #pragma HLS unroll factor=240 // manually added
      v3[j1_init] = v1[i1][j1_init];	// L8 // manually edited
    }
    l_S_k1_0_k1: for (int k1 = 0; k1 < 200; k1++) {	// L10
      l_j1: for (int j1 = 0; j1 < 240; j1++) {	// L11
      #pragma HLS pipeline II=1
      #pragma HLS unroll factor=240
        bool v7 = k1 > i1;	// L12
        if (v7) {	// L13
          float v8 = v0[i1][k1];	// L14
          float v9 = v3[j1];	// L15
          float v10 = v8 * v9;	// L16
