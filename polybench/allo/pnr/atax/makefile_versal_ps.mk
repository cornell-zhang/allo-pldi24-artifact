...
LINK_OUTPUT := $(BUILD_DIR)/kernel_atax.link.xsa
...
VPP_PFLAGS := 
CMD_ARGS = $(BUILD_DIR)/kernel_atax.xclbin
SD_CARD := $(PACKAGE_OUT)
dfx_hw := false
...
# Kernel linker flags
VPP_LDFLAGS_kernel_atax += --optimize 3  --kernel_frequency 300
EXECUTABLE = ./kernel_atax
...
$(TEMP_DIR)/kernel_atax.xo: kernel.cpp
	mkdir -p $(TEMP_DIR)
	v++ -c $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -k kernel_atax --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' $^

$(LINK_OUTPUT): $(TEMP_DIR)/kernel_atax.xo
	mkdir -p $(BUILD_DIR)
	v++ -l $(VPP_FLAGS) $(VPP_LDFLAGS) -t $(TARGET) --platform $(PLATFORM) --temp_dir $(TEMP_DIR) $(VPP_LDFLAGS_kernel_atax) -o'$(LINK_OUTPUT)' $(+)
ifeq ($(EMU_PS), X86)
	v++ -p $(LINK_OUTPUT) $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -o $(BUILD_DIR)/kernel_atax.xclbin
endif
############################## Preparing sdcard ##############################
.PHONY: sd_card
...
ifeq ($(dfx_hw), true)
	v++ -p $(VPP_FLAGS) $(LINK_OUTPUT) -t $(TARGET) --platform $(PLATFORM) -o $(BUILD_DIR)/kernel_atax.xclbin 
	v++ -p $(VPP_PFLAGS) $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(BUILD_DIR)/kernel_atax.xclbin
else
	v++ -p $(VPP_PFLAGS) $(LINK_OUTPUT) $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) --package.sd_file $(EMCONFIG_DIR)/emconfig.json -o $(BUILD_DIR)/kernel_atax.xclbin
endif
